...
BTN1 = Pin(22, Pin.IN, Pin.PULL_UP)    # GP22をBTN1として入力端子（プルアップ）に設定
BTN2 = Pin(23, Pin.IN, Pin.PULL_UP)    # GP23をBTN2として入力端子（プルアップ）に設定
BTN3 = Pin(24, Pin.IN, Pin.PULL_UP)    # GP24をBTN3として入力端子（プルアップ）に設定
...
while True:
    ...
    #  ボタン3処理（カウンタ）
    if BTN3.value() == 0 and state == False: # BTN3がONかつボタン状態Falseなら
        state = True                         # ボタン状態True
        cnt = 0                              # カウントをリセット
    if BTN3.value() == 1:                    # BTN2がOFFなら
        state = False                        # ボタン状態False
